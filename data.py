...
class TravelDataset(Dataset):
    ...
    def __init__(self, args, ori_data_path, dst_data_path, trans_data_path):
        ...
        # ==================== 新增代码 START ====================
        # 创建一个从 POI 整数索引到坐标的张量
        # POI 索引从 1 开始，所以张量大小是 len(self.poi_idx) + 1
        # 索引 0 作为 padding，坐标为 [0, 0]
        self.poi_coord_tensor = torch.zeros(len(self.poi_idx) + 1, 2)
        for poi_str, poi_idx in self.poi_idx.items():
            if poi_str in self.poi_coord_norm:
                self.poi_coord_tensor[poi_idx] = torch.tensor(self.poi_coord_norm[poi_str])
        ...
        last_uid = '0'
        for i in ori_raw:
             # [FIX] 使用切片 [:6] 只取前6列，忽略后面多余的列 (如 POI名称 等)
            uid, cuid, rid, bid, timestamp, std_tag = i[:6] 
...
class ContrastiveDataset(TravelDataset):
    """
    专门用于第一阶段对比学习的数据集。
    读取预生成的 pairs，返回 (Data_A, Data_B)
    """
    def __init__(self, args, ori_data_path, dst_data_path, trans_data_path, pair_file_path):
        # 初始化父类，加载所有基础数据
        super().__init__(args, ori_data_path, dst_data_path, trans_data_path)
        
        # 加载索引对
        with open(pair_file_path, 'rb') as f:
            self.pairs = pickle.load(f) # List of (idx_a, idx_b)
    ...
    def __getitem__(self, index):
        # 获取两个不同的外地行程索引 (同一个用户)
        idx_a, idx_b = self.pairs[index]
        
        # 利用父类方法获取处理好的单条数据
        # item_a 结构: (uid, o_ck, d_ck, ...)
        item_a = super().__getitem__(idx_a)
        item_b = super().__getitem__(idx_b)
        
        # 返回格式：((user_id, home_seq, oot_seq_1, ...), (user_id, home_seq, oot_seq_2, ...))
        # 注意：这里的 user_id 和 home_seq 理论上是一样的
